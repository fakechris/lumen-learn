...
from src.protocol.session import (
    ChapterOutline, CourseStructure, MediaKind, SegmentPlan, SessionOutline, stable_id,
)
...
# 教案的层级
1. 章（chapter）：讲义的自然大块。
2. 课（session）：4~7 分钟，只讲一个核心概念，围绕一个典型认知误区展开，认知阶梯：具象直觉/隐喻 → 认知冲突 → 严谨定义 → 表征 → 应用。
...
只输出一个 JSON 对象：
{
  ...
  "chapters": [{"title": "", "description": "", "sessions": [{
...
OUTLINE_SYSTEM = """你是课程设计专家。下面是一份长讲义的小节索引（id、页码、标题、预览）。请把小节分成 2~8 个"章"，每章是讲义里连续、主题一致的一块。
只输出 JSON：{"chapters": [{"title": "章标题", "description": "一句话", "section_ids": ["s1", "s2"]}]}
...
class PlannedSession(BaseModel):
    ...
    def _segments(cls, v):
        if not 3 <= len(v) <= 8:
            raise ValueError("each session needs 3-8 segments")
        return v
    ...
    for ci, ch in enumerate(planned.chapters, start=1):
        ...
        for s in ch.sessions:
            ...
            sessions.append(SessionOutline(session_id=f"sess_{n}", title=s.title, learning_goal=s.learning_goal,
                                           ...
                                           segments=segments))
        chapters.append(ChapterOutline(chapter_id=f"ch_{ci}", title=ch.title, description=ch.description, sessions=sessions))
