...
def _load_exemplar() -> str:
    ...
    for st in data["steps"][:2]:
        st = dict(st)
        if st.get("widget"):
            st["widget"] = {k: v for k, v in st["widget"].items() if k in ("kind", "title", "task", "layout", "params", "controls")}
        steps.append(st)
...
# 目标画面（必须还原）
- 板书是**电报体笔记**，不是课本段落：一行一个念头，用缩进表示层级，用 → 表示推出。一张板书通常 2~6 行，复杂的推导或对比可以到 8~10 行；再多就拆成两张板书（第二张 layout 用 "newcol"）。
- 讲解**指着板书说**，而且要指到具体位置："看第二行的分母"、"我圈出来的 span"、"右边这张图里红色箭头"、"板书最后一句"。讲解里出现的每个关键词都能在板书或图上找到。指向的说法每步换一种，不要每步都用同一句开头。
- **只能引用本步真实存在的东西**：这一步没有图就不要说"看这张图"，没有教具就不要说"拖滑块"；引用上一步的板书要说"回看上一段的板书"。
...
SYNTH_TIER = os.getenv("LLM_SYNTH_TIER", "fast")  # "pro" writes better prose but is ~5x slower with reasoning models
